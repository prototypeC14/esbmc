  ...
  codet &target_block)
{
  ...
  if (rhs.type() == type_handler_.get_list_type())
  {
    ...
    // If list_type_map is still empty for the LHS (e.g. the list was passed
    // through as a parameter inside the function), fall back to the called
    // function's return-type annotation to determine the element type.
