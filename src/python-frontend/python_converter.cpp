...
exprt python_converter::get_binary_operator_expr(const nlohmann::json &element)
{
  ...
  // to void* so comparisons like `y == False` work when y is Any-typed.
  ...
  bool is_ctor_call)
{
  ...
  {
    // Handle explicit Any-typed annotation assignments (e.g., x: Any = 42)
    // Only applies when the user explicitly wrote `from typing import Any`
    // and annotated `x: Any = value`. Preprocessor-generated AnnAssign nodes
    // with Any annotation (e.g. for-loop variables) are excluded.
    if (
