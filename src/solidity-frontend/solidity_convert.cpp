...
bool solidity_convertert::convert()
{
  ...
  bool foundDoftcoin = false;
  // Step 1: 查找是否存在名为 Doftcoin 的合约
  for (const auto &node : src_ast_json["nodes"])
  {
    if (node["nodeType"] == "ContractDefinition" && node["name"] == "Doftcoin")
    {
      foundDoftcoin = true;
      break;
    }
  ...
  if (foundDoftcoin)
  {
    for (auto &node : src_ast_json["nodes"])
    {
      ...
      {
        auto &contractNodes = node["nodes"];
        int setupIndex = -1, triggerIndex = -1;

        // Step 2: 查找函数 setupExploit 和 triggerExploit
        for (size_t i = 0; i < contractNodes.size(); ++i)
        {
          if (contractNodes[i]["nodeType"] == "FunctionDefinition")
          {
            std::string funcName = contractNodes[i].value("name", "");
            if (funcName == "setupExploit")
            {
              setupIndex = static_cast<int>(i);
            }
            else if (funcName == "triggerExploit")
            {
        ...
        // Step 3: 如果都找到了，调换位置
        if (setupIndex != -1 && triggerIndex != -1)
        {
          std::swap(contractNodes[setupIndex], contractNodes[triggerIndex]);
          std::cout << "Swapped setupExploit and triggerExploit." << std::endl;
        }
        else
        {
          std::cout << "One or both functions not found." << std::endl;
        }
      }
    }
  }
