...
// ============================================================
// File-writing helpers — C and C++ are cleanly separated below
// ============================================================
...
/// Write a plain C test case file to path.
/// When index/total are non-zero a "Test case N of M" comment is added.
  ...
  size_t total = 0)
{
  ...
  // __VERIFIER_assume is a no-op: the concrete values already satisfy all
  // assumptions. Providing a definition here avoids linker errors.
...
/// All functions are placed inside extern "C" for correct linkage, and
/// bool is used instead of _Bool.
...
/// declaration for them.  When the generated CTest cases are compiled the
/// compiler requires explicit declarations to resolve the calls in the
/// original source file.  This header is force-included by CMake via
/// target_compile_options(-include ...) so no modification to the original
/// source file is required.
///
/// WHAT YOU CAN DO
/// ---------------
/// If you prefer explicit declarations in your own code, you can simply add
///   #include "esbmc_verifier.h"
/// at the top of your source file.  Including the header both manually and
/// via -include is harmless; duplicate declarations are permitted in both C
/// and C++.
...
/// Write CMakeLists.txt for a C or C++ project.
///
/// Also generates esbmc_verifier.h and force-includes it via
/// target_compile_options.  This ensures the original source file compiles
/// correctly even when it has no explicit declarations for __VERIFIER_*
/// functions (required in strict C99+; needed for extern "C" linkage in C++).
...
void ctest_generator::generate() const
{
  ...
  for (size_t i = 0; i < unique_indices.size(); ++i)
  {
    ...
    if (cpp_mode)
      write_cpp_test_file(test_file, type_values, c_types, i + 1, unique_indices.size());
    else
      write_c_test_file(test_file, type_values, c_types, i + 1, unique_indices.size());
  }

  write_cmake(cpp_mode, "ESBMCGeneratedTests", source_file, target_names, test_file_names);

  log_status(
    "Generated {} CTest test case(s) with CMakeLists.txt", unique_indices.size());
}
  ...
  const namespacet &ns)
{
  ...
  write_cmake(cpp_mode, "ESBMCGeneratedTest", src_file, {"test_case"}, {test_file_name});
