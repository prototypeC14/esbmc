...
/// Write esbmc_verifier.h containing extern "C" declarations for all
/// __VERIFIER_* functions. This is force-included via CMake -include so that
/// C++ source files that call these functions without their own declarations
/// will compile correctly.
static void write_verifier_header(bool cpp_mode)
{
  std::ofstream header("esbmc_verifier.h");
  header << "// Auto-generated by ESBMC " << ESBMC_VERSION << "\n";
  header << "// Forward declarations for __VERIFIER_* functions\n";
  header << "#pragma once\n\n";
  header << "#ifdef __cplusplus\n";
  header << "extern \"C\" {\n";
  header << "#endif\n\n";
  header << "/* Nondeterministic value functions */\n";
  header << "char __VERIFIER_nondet_char(void);\n";
  header << "short __VERIFIER_nondet_short(void);\n";
  header << "int __VERIFIER_nondet_int(void);\n";
  header << "long long __VERIFIER_nondet_long(void);\n";
  header << "unsigned char __VERIFIER_nondet_uchar(void);\n";
  header << "unsigned short __VERIFIER_nondet_ushort(void);\n";
  header << "unsigned int __VERIFIER_nondet_uint(void);\n";
  header << "unsigned long long __VERIFIER_nondet_ulong(void);\n";
  header << "float __VERIFIER_nondet_float(void);\n";
  header << "double __VERIFIER_nondet_double(void);\n";
  if (cpp_mode)
    header << "bool __VERIFIER_nondet_bool(void);\n";
  else
    header << "_Bool __VERIFIER_nondet_bool(void);\n";
  header << "void *__VERIFIER_nondet_pointer(void);\n\n";
  header << "/* Assumption function */\n";
  header << "void __VERIFIER_assume(int cond);\n\n";
  header << "#ifdef __cplusplus\n";
  header << "} /* extern \"C\" */\n";
  header << "#endif\n";
  header.close();
...
void ctest_generator::generate() const
{
  ...
  // For C++ mode, generate esbmc_verifier.h so that source files calling
  // __VERIFIER_* functions without their own extern "C" declarations can
  // compile. This header is force-included via CMake target_compile_options.
  if (cpp_mode)
    write_verifier_header(cpp_mode);
  ...
  for (size_t i = 0; i < test_cases.size(); ++i)
  {
    ...
    // Always define __VERIFIER_assume: the generated concrete values already
    // satisfy all assumptions so this can be a no-op. Providing the definition
    // here avoids linker errors for programs that call __VERIFIER_assume.
    test_file << "void __VERIFIER_assume(int cond) { (void)cond; }\n\n";
  ...
  for (size_t i = 0; i < test_file_names.size(); ++i)
  {
    ...
    cmake_file << "add_test(NAME " << test_name << " COMMAND " << test_name
               << ")\n";
    // For C++ targets, force-include esbmc_verifier.h so that C++ source
    // files which call __VERIFIER_* without their own extern "C" declarations
    // will compile correctly.
    if (cpp_mode)
      cmake_file
  ...
  const namespacet &ns)
{
  ...
  // For C++ mode, generate esbmc_verifier.h (see generate() for rationale).
  if (cpp_mode)
    write_verifier_header(cpp_mode);
  ...
  // Always define __VERIFIER_assume (see generate() for rationale).
  test_file << "void __VERIFIER_assume(int cond) { (void)cond; }\n\n";
  ...
  if (cpp_mode)
    cmake_file
      << "target_compile_options(test_case"
      << " PRIVATE -include ${CMAKE_CURRENT_SOURCE_DIR}/esbmc_verifier.h)\n";
