...
std::string ctest_generator::type_to_verifier_string(const type2tc &type) const
{
  if (is_signedbv_type(type) || is_unsignedbv_type(type))
  {
    ...
    {
      if (width == 8) return "uchar";
      if (width == 16) return "ushort";
      if (width == 32) return "uint";
      if (width == 64) return "ulong";
      return "uint";  // Default
    }
  }
  else if (is_floatbv_type(type))
...
void ctest_generator::generate(const std::string &output_dir) const
{
  ...
  for (size_t i = 0; i < test_cases.size(); ++i)
  {
    ...
    test_file << "// This file provides concrete implementations of __VERIFIER_nondet_* functions\n\n";
    ...
    for (const auto &[verifier_type, values] : type_values)
    {
      ...
      test_file << c_type << " __VERIFIER_nondet_" << verifier_type << "(void) {\n";
  ...
  cmake_file << "if(ENABLE_COVERAGE AND CMAKE_C_COMPILER_ID MATCHES \"GNU|Clang\")\n";
  ...
  cmake_file << "# Each test case is compiled with the original source + test case implementation\n";
  for (size_t i = 0; i < test_file_names.size(); ++i)
  {
    std::string test_name = "test_case_" + std::to_string(i + 1);
    cmake_file << "add_executable(" << test_name << " " << src_file_name << " " << test_file_names[i] << ")\n";
  ...
  const namespacet &ns)
{
  ...
  cmake_file << "add_executable(test_case " << src_file_name << " test_case.c)\n";
