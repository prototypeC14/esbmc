...
/// correctly under strict C99+ even when it has no explicit declarations for
/// __VERIFIER_* functions.
  ...
  const std::vector<std::string> &test_files)
{
  ...
  cmake << "if(ENABLE_COVERAGE AND CMAKE_C_COMPILER_ID MATCHES \"GNU|Clang\")\n";
  ...
  for (size_t i = 0; i < target_names.size(); ++i)
  {
    ...
    // __VERIFIER_* declarations it needs even if it does not declare them
    // itself (required under strict C99+).
...
/// Also generates esbmc_verifier.h and force-includes it via
/// target_compile_options.  This ensures the original source file compiles
/// correctly even when it has no explicit extern "C" declarations for
/// __VERIFIER_* functions.
  ...
  const std::vector<std::string> &test_files)
{
  ...
  for (size_t i = 0; i < target_names.size(); ++i)
  {
    ...
    // "C" declarations it needs even if it does not include them itself.
    cmake << "target_compile_options(" << target_names[i]
          << " PRIVATE -include ${CMAKE_CURRENT_SOURCE_DIR}/esbmc_verifier.h)"
             "\n\n";
  }
...
void ctest_generator::generate() const
{
  ...
  if (cpp_mode)
    write_cpp_cmake("ESBMCGeneratedTests", source_file, target_names, test_file_names);
  else
